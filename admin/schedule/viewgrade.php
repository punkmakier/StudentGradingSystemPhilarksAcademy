...
<div class="row">
 ...
 <?php 
$sql =" SELECT * FROM  `schoolyr` sy, `course` c
       WHERE  sy.`COURSE_ID`=c.`COURSE_ID` AND sy.`IDNO`=".$IDNO;
$mydb->setQuery($sql);
...
$cur = $mydb->loadSingleResult(); 
 ?>
  ...
  <div class="row">
    <div class="col-lg-12">
  	<div class="col-md-6">
  		<h4>Grade Level :<?php echo $singlecourse->COURSE_LEVEL;?> </h4>
  	</div>
...
<div class="row">
      <div class="col-lg-12"> 
                  <h3 class="page-header">Student Subjects <span><a class="btn btn-primary" href="controller.php?action=submitgrades&idno=<?php echo $IDNO;?>">Submit Grades</a></span></h3>
				...
				<table id="dash-table" class="table table-striped table-bordered table-hover table-responsive"   cellspacing="0">
				  ...
				  <tbody>
				  	...
				  	//  `FIRST`, `SECOND`, `THIRD`, `FOURTH`, `AVE`, `DAY`, `G_TIME`, `ROOM`, `REMARKS`, `COMMENT`
						...
						WHERE st.`IDNO`=g.`IDNO` and g.`SUBJ_ID`=s.`SUBJ_ID`  and s.`SUBJ_ID`=ss.`SUBJ_ID` AND g.`IDNO`=ss.`IDNO` and s.COURSE_ID='".$res->COURSE_ID."' AND st.`IDNO`=".$IDNO;
				  		$mydb->setQuery($sql);
						...
						foreach ($cur as $result) {
				  		...
				  		echo '<td align="center" > <a  title="Edit" href="addmodalgrades.php?id='.$result->SUBJ_ID.'&IDNO='.$result->IDNO.'&gid='.$result->GRADE_ID.'" data-toggle="lightbox" data-title="Add Grades" >  <span class="fa fa-plus fw-fa"></span> Add gardes</a>
				  					  </td>';

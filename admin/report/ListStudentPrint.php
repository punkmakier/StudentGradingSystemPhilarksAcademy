...
<div class="wrapper">
  ...
  <section class="invoice">
    ...
    <div class="row">
      <div class="col-xs-12">
        <h4 class="page-header ">
          <i class="fa fa-globe"></i><?php SiteTitle();?>
    ...
    <h5 align="center"><?php echo isset($_POST['Course']) ? "Grade Level :". $_POST['Course'] ." || SY :".$_POST['SY']: ''; ?></h5></div>
    <!-- info row --> 
     <!-- Table row --> 
          <table class="table table-bordered  table-striped" style="font-size:11px" cellspacing="0" >
            ...
            if ($_POST['Course']=='All') {
              # code...
                 ...
                 if($gender==''){
                       $sql ="SELECT * FROM `schoolyr` sy, `tblstudent`  s ,`course` c 
                        WHERE sy.`IDNO`=s.`IDNO` AND s.`COURSE_ID`=c.`COURSE_ID` AND sy.AY = '" .$_POST['SY']. "'";
                  }else{
                       $sql ="SELECT * FROM `schoolyr` sy, `tblstudent`  s ,`course` c 
                        WHERE sy.`IDNO`=s.`IDNO` AND s.`COURSE_ID`=c.`COURSE_ID`   AND SEX = '{$gender}'  AND sy.AY = '" .$_POST['SY']. "'";
                  }
              ...
              <?php  
                 ...
                 if($gender==''){
                       $sql ="SELECT * FROM `schoolyr` sy, `tblstudent`  s ,`course` c 
                        WHERE sy.`IDNO`=s.`IDNO` AND s.`COURSE_ID`=c.`COURSE_ID`  AND c.COURSE_ID = '" . $_POST['Course'] ."' AND sy.AY = '" .$_POST['SY']. "'";
                  }else{
                       $sql ="SELECT * FROM `schoolyr` sy, `tblstudent`  s ,`course` c 
                        WHERE sy.`IDNO`=s.`IDNO` AND s.`COURSE_ID`=c.`COURSE_ID`   AND SEX = '{$gender}'  AND c.COURSE_ID = '" . $_POST['Course'] ."'  AND sy.AY = '" .$_POST['SY']. "'";
                  }

	...
	<div class="table-responsive"> 
		<table class="table " >
			...
			<td><label>Contact No.</label></td>
				<td colspan="2"><input required="true" pattern="[0-9]{11}" class="form-control input-md" id="CONTACT" name="CONTACT" placeholder="09XXXXXXXXX" type="tel" maxlength="11" value="<?php echo isset($_SESSION['CONTACT']) ? $_SESSION['CONTACT'] : ''; ?>">
							</td>
			...
			<td><label>Grade Level</label></td>
				<td colspan="2">
					...
					<select class="form-control input-sm" name="COURSE">
								...
								$mydb->setQuery("SELECT * FROM `course` WHERE COURSE_LEVEL LIKE '%Nursery 1%'");
								$cur = $mydb->loadResultList();

								foreach ($cur as $result) {
								  echo '<option value='.$result->COURSE_ID.' selected>'.$result->COURSE_LEVEL.' </option>';
			...
			<tr>
				...
				<td colspan="2" class="password-container">
						<input required="true"  class="form-control input-md" id="U_PASS" name="PASS" placeholder="Password" type="password"value="<?php echo isset($_SESSION['PASS']) ? $_SESSION['PASS'] : ''; ?>">
						 <i class="far fa-eye" id="togglePassword" style="margin-left: 0px; margin-top: 8px; cursor: pointer;"></i>
				</td>
			</tr>
			<tr>
				...
				<td><label>Photo of Birth Certificate</label></td>
